...
{
    [Serializable]
    public class AssetStreamingManager : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            ReleaseArea();
            BadgeArea();
            AssetStreamingUrlArea();

            GUILayout.Label("");
            GUILayout.Label("  ( You can go to Window/Unity Online Service/CDN/Settings to personalize your configuration. )");
        ...
        void CheckInitInfo()
        {
            if (string.IsNullOrEmpty(Parameters.uosAppId) || string.IsNullOrEmpty(Parameters.uosAppSecret))
            {
                ...
                GUILayout.Label("Plesse set uos app id & secret before using. (Edit -> Project Settings... -> Unity Online Service)", s);
                GUILayout.Label("If you have not got uos app id & secret already. Please go to uos dashboard first. (Window -> Unity Online Service -> CDN -> Go to Dashboard)", s);
                GUILayout.EndHorizontal();
            }
            else if (string.IsNullOrEmpty(Parameters.projectGuid) || string.IsNullOrEmpty(Parameters.backend))
            {
                if (!(string.IsNullOrEmpty(UosAppInfo.getProjectGuid()) || string.IsNullOrEmpty(UosAppInfo.getbackend())))
                {
                    Parameters.projectGuid = UosAppInfo.getProjectGuid();
                    Parameters.backend = UosAppInfo.getbackend();
                }
                else
                {
                    GUILayout.BeginHorizontal();
                    ...
                    if (projectInfo != null)
                    {
                        string projectGuid = projectInfo.UnityProjectGuid;
                        string backend = projectInfo.Provider;
            ...
            {
                if (string.IsNullOrEmpty(UosAppInfo.getProjectGuid()))
                {
                    UosAppInfo.SaveProjectGuid(Parameters.projectGuid);
                }
                if (string.IsNullOrEmpty(UosAppInfo.getbackend()))
                {
                    UosAppInfo.Savebackend(Parameters.backend);
                }
            }
